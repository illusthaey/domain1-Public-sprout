...
(() => {
  ...
  function renderCalendar() {
    ...
    weeks.forEach((week, rIdx) => {
      ...
      week.forEach((dateObj, cIdx) => {
        ...
        cb.type = "checkbox";
        cb.checked = !!a.on;
        cb.dataset.grid = "calendar";
        cb.dataset.r = String(rIdx);
        cb.dataset.c = String(cIdx);
        cb.dataset.date = dateISO;

        cb.addEventListener("change", () => {
          ensureAttendanceKey(dateISO).on = cb.checked;
          computeAndRender();
          scheduleSaveDebounced();
        });
  ...
  function updateCalendarHints(ym) {
    const showManual = !!state.ui.showManualDailyHours;
    // DOM 전체를 순회하며 date를 가진 input을 찾는 방식(간단/안전)
    document.querySelectorAll('input[data-grid="calendar"][data-date]').forEach((cb) => {
  ...
  function fillCalendarBySchedule() {
    ...
    // UI 반영: 체크박스만 갱신(전체 rerender로 간단 처리)
    renderCalendar();
  }
  ...
  function setupGridNavigation() {
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key === " " && (t instanceof HTMLInputElement) && t.type === "checkbox") {
        e.preventDefault();
        t.checked = !t.checked;
